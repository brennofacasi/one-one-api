...
meeting_bp = APIBlueprint('meeting', __name__, url_prefix='/meeting')

meeting_tag = Tag(
    name='Meetings', description='Adição, visualização e atualização de reuniões.')


@meeting_bp.get('/', tags=[meeting_tag], responses={"200": MeetingViewSchema})
def get_meetings():
    # TO DO - Implementar
    return 'Deu certo'


@meeting_bp.post('/', tags=[meeting_tag])
